...
void SetupRC(void){
	...
	glTexImage2DMultisample(GL_DRAW_FRAMEBUFFER, 8, GL_DEPTH_COMPONENT24, screenWidth, screenHeight, GL_FALSE);

	glGenTextures(1, msTexture);
	glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, msTexture[0]);
	glTexImage2DMultisample(GL_DRAW_FRAMEBUFFER, 8, GL_RGBA8, screenWidth, screenHeight, GL_FALSE);
